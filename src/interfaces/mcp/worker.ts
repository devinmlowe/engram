 ...
 *
 * Protocol: receives `{ id, tool, args, context? }`, answers `{ id, ok, result }` or
...
import { argsDigest, type ToolCallContext } from "./dispatch.js";
...
port.on("message", (message: unknown) => {
  ...
  const { id, tool, args, context } = request as WorkerRequest;
  ...
  handleToolCall(tool, args, context as ToolCallContext | undefined).then(
    (result) => {
