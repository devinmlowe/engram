...
- [sync-manifests.cjs](./sync-manifests.cjs) — Dependency-free: writes the `package.json` version (and the `npx -y @devinmlowe/engram@<v>` pin) into `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json` and `server.json`; `--check` exits 1 listing every mismatch and asserts the shape each consumer needs (inline `mcpServers`, marketplace `source: "./"`, registry `io.github.*` name = `package.json` `mcpName`, `files[]` carrying the plugin). Runs from the npm `version` hook, CI and the release workflow (#58)
