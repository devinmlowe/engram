...
- [sync-manifests.cjs](./sync-manifests.cjs) — Dependency-free: writes the `package.json` version (and the `npx -y @devinmlowe/engram@<v>` pin) into `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json` and `server.json`; `--check` exits 1 listing every version mismatch, a `package.json` `mcpName` that differs from the `server.json` name, or a registry description over 100 characters. Manifest shape is `claude plugin validate` / `mcp-publisher validate`'s job (both run in CI). Runs from the npm `version` hook, CI and the release workflow (#58)
