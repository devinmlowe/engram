...
ensure_env_file() {
    ...
    (
        umask 077
        {
            ...
            echo "# Generic OpenAI-compatible route (OpenAI, LiteLLM, self-hosted): the key is read from the env var NAMED below"
            echo "#ENGRAM_OPENAI_BASE_URL=https://api.openai.com/v1"
            echo "#ENGRAM_OPENAI_MODEL="
            echo "#ENGRAM_OPENAI_API_KEY_ENV=OPENAI_API_KEY"
